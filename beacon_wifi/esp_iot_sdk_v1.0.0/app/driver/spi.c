...
spi_lcd_mode_init(uint8 spi_no)
{
    uint32 regvalue;
    ...
    WRITE_PERI_REG(SPI_CLOCK(spi_no),
                   ((100&SPI_CLKDIV_PRE)<<SPI_CLKDIV_PRE_S)|
...
spi_master_init(uint8 spi_no)
{
    ...
    CLEAR_PERI_REG_MASK(SPI_USER(spi_no), SPI_FLASH_MODE | SPI_CK_OUT_EDGE | SPI_USR_COMMAND);
	SET_PERI_REG_MASK(SPI_USER(spi_no), SPI_FLASH_MODE);
    //clear Daul or Quad lines transmission mode
    CLEAR_PERI_REG_MASK(SPI_CTRL(spi_no), SPI_QIO_MODE|SPI_DIO_MODE|SPI_DOUT_MODE|SPI_QOUT_MODE);

    WRITE_PERI_REG(SPI_CLOCK(spi_no),
					((20&SPI_CLKDIV_PRE)<<SPI_CLKDIV_PRE_S)|
                   ((3&SPI_CLKCNT_N)<<SPI_CLKCNT_N_S)|
                   ((1&SPI_CLKCNT_H)<<SPI_CLKCNT_H_S)|
                   ((3&SPI_CLKCNT_L)<<SPI_CLKCNT_L_S)); //clear bit 31,set SPI clock div
	...
	//spi_mast_byte_write(spi_no,0xff);
}
...
*******************************************************************************/
uint8 spi_mast_byte_write(uint8 spi_no,uint8 data)
{
    uint32 regvalue;
    uint8  rev_data;
    if(spi_no>1) 		return; //handle invalid input number
    ...
    CLEAR_PERI_REG_MASK(SPI_USER(spi_no), SPI_USR_MISO|SPI_USR_ADDR|SPI_USR_COMMAND|SPI_USR_DUMMY);
   ...
   // WRITE_PERI_REG(SPI_W0(spi_no), (uint32)0xff);               
 ...
 *				uint8 data- transmitted data
*******************************************************************************/
uint8 spi_mast_byte_read(uint8 spi_no)
{
    uint32 regvalue;
    uint8  rev_data;
    if(spi_no>1) 		return; //handle invalid input number
    ...
    CLEAR_PERI_REG_MASK(SPI_USER(spi_no), SPI_USR_ADDR|SPI_USR_DUMMY|SPI_USR_MOSI|SPI_USR_COMMAND);
  ...
  //  WRITE_PERI_REG(SPI_USER2(spi_no),
   //                ((7&SPI_USR_COMMAND_BITLEN)<<SPI_USR_COMMAND_BITLEN_S)|6);
	...
	rev_data = READ_PERI_REG(SPI_W0(spi_no)); //Assuming data in is written to MSB. TBC


   // rev_data=(uint8)(READ_PERI_REG(SPI_W0(spi_no))&0xff);
...
disp_spi_data()
{
    uint8 i = 0;
    for(i=0; i<32; i++) {
        CLING_DEBUG("data %d : 0x%02x\n\r",i,spi_data[i]);
    }
    //CLING_DEBUG("d31:0x%02x\n\r",spi_data[31]);
}
...
spi_task(os_event_t *e)
{
    ...
    case MOSI:
        ...
        break;
    case STATUS_R_IN_WR :
        CLING_DEBUG("SR ERR in WRPR,Reg:%08x \n",e->par);
        break;
    case STATUS_W:
        CLING_DEBUG("SW ERR,Reg:%08x\n",e->par);
        break;
    case TR_DONE_ALONE:
        CLING_DEBUG("TD ALO ERR,Reg:%08x\n",e->par);
        break;
    case WR_RD:
        CLING_DEBUG("WR&RD ERR,Reg:%08x\n",e->par);
        break;
    case DATA_ERROR:
        CLING_DEBUG("Data ERR,Reg:%08x\n",e->par);
        break;
    case STATUS_R_IN_RD :
        CLING_DEBUG("SR ERR in RDPR,Reg:%08x\n",e->par);
        break;
    default:
        break;
    }
...
spi_test_init()
{
    CLING_DEBUG("spi init\n\r");
    spi_slave_init(HSPI);
    CLING_DEBUG("gpio init\n\r");
    gpio_init();
    CLING_DEBUG("spi task init \n\r");
    spi_task_init();
#ifdef SPI_MISO
    CLING_DEBUG("spi miso init\n\r");
    set_miso_data();
#endif
