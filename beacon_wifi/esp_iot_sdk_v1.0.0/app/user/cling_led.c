...
start_upgrade_trigger_key_install(void)
{
    ...
    single_key[0] = key_init_single(CLING_KEY_UPGRADE_IO_NUM, CLING_KEY_UPGRADE_IO_MUX, CLING_KEY_UPGRADE_IO_FUNC,
...
cling_platform_upgrade_begin(struct espconn *pespconn, struct upgrade_server_info *server)
{
    ...
    os_sprintf(server->url, "GET /Files/%s HTTP/1.0\r\nHost: "IPSTR":%d\r\n"PHEADERBUFFER"",
...
*******************************************************************************/
#if 1
bool upgrade_finish_callback(CLASS(cling_fota) *arg, void (*upgrade_accompolished_callback)(struct upgrade_server_info)){
	struct fota_paramater *fota_data = (struct fota_paramater*)(arg->user_data);
...
*******************************************************************************/
#if 1
bool upgrade_startup_callback(CLASS(cling_fota) *arg, void (*upgrade_startup_callback)()){
	struct fota_paramater *fota_data = (struct fota_paramater*)(arg->user_data);
	/*check  if user data is valid or not*/
