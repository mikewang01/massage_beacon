...
timer_get(struct jsontree_context *js_ctx)
{
...
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR
timer_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
#if 1
...
LOCAL struct jsontree_callback status_callback =
    JSONTREE_CALLBACK(timer_get, NULL);
...
LOCAL struct jsontree_callback backtime_callback =
    JSONTREE_CALLBACK(NULL, timer_set);
