...
#ifdef UPGRADE_DEBUG
#define BLE_UPGRADE_DBG os_printf
#else
#define BLE_UPGRADE_DBG
#endif
...
#define  BLE_UPGRADE_FLAG_FINISH (UPGRADE_FLAG_FINISH + 1)
/*********************************************************************
* TYPEDEFS
*/
...
//user_flow_send ble_upgrade_flow = NULL;
...
{
    if (ble_upgrade_flag_check() != UPGRADE_FLAG_START) {
        ble_upgrade_deinit();
    }
...
{
    BLE_UPGRADE_DBG("upgrade_check\n");

    if (ble_upgrade_flag_check() != BLE_UPGRADE_FLAG_FINISH) {
        totallength = 0;
        sumlength = 0;
...
upgrade_download(void *arg, char *pusrdata, unsigned short length)
{
    ...
    if (totallength == 0 && (ptr = (char *)os_strstr(pusrdata, "\r\n\r\n")) != NULL &&
        ...
        length -= 4;
        totallength += length;
        BLE_UPGRADE_DBG("ble upgrade file download start.\n");
        ble_upgrade(ptr + 4, length);
        ...
        if (ptr != NULL) {
            ...
            if (ptmp2 != NULL) {
                ...
                BLE_UPGRADE_DBG("ble sumlength failed\n");
            }
        } else {
            BLE_UPGRADE_DBG("ble Content-Length: failed\n");
        }
    } else {
        totallength += length;
        os_printf("totallen = %d\n",totallength);
        ble_upgrade(pusrdata, length);
    }

    if (totallength == sumlength) {
        BLE_UPGRADE_DBG("ble upgrade file download finished.\n");
		//espconn_disconnect(pespconn);
        ble_upgrade_flag_set(BLE_UPGRADE_FLAG_FINISH);
        totallength = 0;
        sumlength = 0;
...
upgrade_connect_cb(void *arg)
{
    ...
    BLE_UPGRADE_DBG("ble upgrade_connect_cb\n");
    os_timer_disarm(&upgrade_10s);
...
{
    BLE_UPGRADE_DBG("ble upgrade_connect\n");
    ...
    ble_upgrade_init();
    ble_upgrade_flag_set(UPGRADE_FLAG_START);
...
/******************************************************************************
 * FunctionName : ble_upgrade_set_user_flow
 * Description  : set firmware flow direction from server
 * Parameters   : pfunc specific flow control
 * Returns      : none
*******************************************************************************/
bool ICACHE_FLASH_ATTR
ble_upgrade_set_user_flow(bool (*pfunc)(char*, size_t))
{
	if (pfunc != NULL){
		ble_upgrade_flow = pfunc;
	}else{
		return FALSE;
	}
	return TRUE;
...
bool ICACHE_FLASH_ATTR
#ifdef UPGRADE_SSL_ENABLE
ble_upgrade_start_ssl(struct upgrade_server_info *server)
#else
ble_upgrade_start(struct upgrade_server_info *server)
#endif
{
    if (ble_upgrade_flag_check() == UPGRADE_FLAG_START) {
        return false;
